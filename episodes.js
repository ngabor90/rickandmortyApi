...
const fetchEpisodes = async () => {
  try {
    ...
    if (!response.ok) throw new Error("Hiba történt az epizódok lekérésekor.");

    const json = await response.json();
    const maxPageNumber = json.info.pages;

    const buttonLoc = document.getElementById("paginationButton");
    ...
    // Táblázat feltöltése epizód adatokkal
    json.results.forEach(episode => {
      const tr = document.createElement("tr");

      ["name", "air_date", "episode"].forEach(key => {
        const td = document.createElement("td");
        td.textContent = episode[key];
        tr.appendChild(td);
      });
